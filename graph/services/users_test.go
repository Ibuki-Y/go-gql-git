...
func TestGetUserByID(t *testing.T) {
	...
	mockSetup := func(mock sqlmock.Sqlmock, id, name string) {
		...
		mock.ExpectQuery(".*").WithArgs(id).WillReturnRows(sqlmock.NewRows(columns).AddRow(id, name))
	}
	...
	for _, tt := range tests {
		t.Run(tt.title, func(t *testing.T) {
			...
			out, err := srv.GetUserByID(ctx, tt.id)
			if err != nil {
				t.Error(err)
			}
			if diff := cmp.Diff(tt.expected, out); diff != "" {
